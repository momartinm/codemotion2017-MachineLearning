...
def showResults( data_y_pred, data_y_test ):
        ...
        # Plot outputs
      	plt.plot(data_y_test, data_y_pred, 'ro')
	plt.plot([0,10000000],[0,50], 'g-')
	plt.xlabel('pred')
	plt.ylabel('test')
	plt.show()
...
def main( argv ):
	if (len(argv) == 2):
		incrementalLearning(argv[1], 65536, SGDRegressor(loss='squared_loss', penalty='l1', max_iter=25), 0)
	elif (len(argv) == 3):
		incrementalLearning(argv[1], 65536, SGDRegressor(loss='squared_loss', penalty='l1', max_iter=25), argv[2])
	else:
		print("You should insert the file path as parameter")
	pass

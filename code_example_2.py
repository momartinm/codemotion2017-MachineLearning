...
def showResults( data_y_pred, data_y_test, name ):
...
def incrementalLearning( path, buffer_size, model, example_mode, period, output_name):
		...
		while True:
			...
			for line in lines:
				...
				if (current_year < test_year):	
				        if year - start_year_period == period or year == test_year:
					        # Fit linear model with Stochastic Gradient Descent for a group of years
						model.partial_fit(np.array(data_X_train).astype(np.float), np.array(data_y_train).astype(np.float))
						print("Partial fit from %s to %s" % (start_year_period, year-1))
						current_year = year
						start_year_period = year
                                                # Clean data to collect new year
						data_X_train = []
						data_y_train = []
					data_X_train.append(generateExample(data, cities, example_mode))
					data_y_train.append(data[1])
			        # Generating training data
				elif (current_year == test_year):
					data_X_test.append(generateExample(data, cities, example_mode))
					data_y_test.append(data[1])						
        
	data_y_prediction = model.predict(np.array(data_X_test).astype(np.float))
	print('Coefficients: ', model.coef_)
	showResults(data_y_prediction, np.array(data_y_test).astype(np.float), output_name)		
...
def main( argv ):
        ...
        parser.add_argument('-p', type=int, nargs=1, help='Period: number of year for partial fit', default=[1], required=False)
        parser.add_argument('-o', type=str, nargs=1, help='output file', default='output_filename', required=False)

        args = parser.parse_args(argv[1:])
        incrementalLearning(args.f[0], 65536, SGDRegressor(loss='squared_loss', penalty='l1', max_iter=25), args.m[0], args.p[0], args.o[0])
